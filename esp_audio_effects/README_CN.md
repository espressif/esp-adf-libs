...
Espressif Audio Effects（ESP_AUDIO_EFFECTS）是乐鑫为 SoC 打造的官方音频处理模块集合，提供一系列专业且高性能的音频处理算法，可用于修改、增强或塑造音频信号的特性。当前支持的模块包括：自动电平控制（ALC）、采样率转换、位深转换、声道转换、均衡（EQ）、数据交织（Data Weaver）、混音（Mixer）、淡入淡出（Fade）、Sonic 变速/变调处理、动态范围控制（DRC）、多频段动态范围压缩（MBC）以及啸叫抑制（HOWL）。
