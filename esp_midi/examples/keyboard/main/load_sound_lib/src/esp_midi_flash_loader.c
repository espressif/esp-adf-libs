...
esp_midi_err_t esp_midi_flash_loader_noteon(esp_midi_lib_loader_handle_t lib_handle, esp_midi_lib_sample_desc_t *sample_desc)
{
    ...
    esp_midi_err_t ret = ESP_MIDI_ERR_OK;
    int32_t instr = 0, note_val = 0;
    int wav_number = MIDI_TOTAL_NOTE_NUMBER;
    int instrument_number = instrument_info_table[midi_player_get_current_instr_index()].instrument;

    for (int i = 0; i < wav_number; i++) {
        instr = wav_index_table[i].instrument;
        note_val = wav_index_table[i].note;
        if (note_val == sample_desc->note && instr == instrument_number) {
